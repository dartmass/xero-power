...
| 42 | 別組織への着地を検出する | ✅ | **撮影中に実機で発生した事故。** パレット39件中23件が組織IDを持たず、Xeroがセッションから組織を決めるため着地先が不定になる。不一致時に赤い警告バーを出す実装を追加し、一致/不一致の両方向を実機で3回確認。**予防（組織URLに寄ってセッションを固定）は実機で否定された** — 下記参照 |
| 43 | 組織名が略称で上書きされる | ✅ | ナビは画面が狭いと `Demo Company (Global)` → `DCG`、`bbb` → `b` と略す。警告文が「Xero opened DCG, not b.」になり、取り違えを防ぐ場面で読めなかった。長いほうを残すよう修正、9件のテストで確認 |
| 44 | 銀行照合のトラッキング必須化（Practice Pro） | ✅ | Createでトラッキング未選択のままOKを押せない。**キーボード・マウスの両経路を実機確認。** Region欄が赤枠になり、画面下に理由が出る。実DOMを採取して実装、10件の自動テスト＋実機 |
| 45 | Create中に候補を開いたままCmd+Enter | ✅ | ExtJSのコンボは開いたままだと値が未確定で、Xeroが「You must set a payee, account and tax rate」を出していた。**Whyは必須ではない**ので、Whatで選び切らずに保存する人は必ず踏む。確定前にblurして値を確定させるよう修正。**候補を開いたまま／通常の順序、どちらも通ることを実機確認** |
...
## 未実施: QA 44 — 銀行照合のトラッキング必須化

### なぜ作ったか

Xero自身がMTD for Income Taxの事業切り分けに「トラッキングカテゴリを使え」と案内しており、
**トラッキングの無い明細は四半期申告の集計から除外される**。そのトラッキングの必須化を
Xeroは2025年7月に「作らない」と明言した。

承認ワークフロー製品（ApprovalMax等）は「書類を吸い上げ→承認→Xeroへ戻す」構造なので、
**承認という概念が無い銀行照合には構造上届かない**（対応書類は Invoices / PO / Credit Notes /
Expenses のみと一次資料で確認）。個人事業主・大家の取引はほぼ銀行照合から生まれるため、
MTDの露出はここに集中する。

### 実DOM（2026-08-12 Demo Companyで採取）
...
- ⚠️ **idは `ext-comp-1011` のようなExtJSの自動採番。セレクタに使わないこと**
- ⚠️ **「valueが空か」では判定できない。** 未選択でもカテゴリ名が入っている
- `div.tracking` の数 = その組織のトラッキングカテゴリ数
- クラスで判定するので**カテゴリ名が "Region" でない組織でも効く**

### テスト手順

1. Practice Proにして `Approval guardrail — require tracking categories` をON
2. Bank Recで明細を選び `C`（Create）
3. Who / What を入れ、**Regionは空のまま** `Cmd/Ctrl+Enter`
4. 止まること・Region欄が赤枠になること・画面下に理由が出ることを確認
5. Regionを選んで再度 `Cmd/Ctrl+Enter` → 通ること
6. 同じことを**マウスでOKボタンを押して**も確認する（経路が別）
7. `M`（Match）では止まらないことを確認する（トラッキング欄が無いため）
8. 設定をOFFにすると止まらないことを確認する

### 仕様通りの結果

- トラッキング未選択のままでは照合できない
- 赤枠が付くのは足りていないカテゴリ欄だけ
- 入力すると赤枠が消える
- Match・Transferでは何も起きない
- FreeとSolo Proでは何も起きない
...
#### 予防: 「組織URLに寄る」案は実機で否定された（2026-08-12）

`/app/{orgId}/homepage` へ寄ってから旧URLへ移動すればセッションを固定できる、という案を実測した。
**効かない。**

| 出発 | Ctrl+K → New Bill の着地 | 警告バー |
|---|---|---|
| Demo Company のホーム | Demo Company | 出ない（一致） |
| bbb のホーム | **Demo Company** | 出る（不一致） |
| aaa のホーム | aaa | 出ない（一致） |
| bbb のホーム | **aaa** | 出る（不一致） |

上2行はデモ会社の特殊セッション（"You're currently using the demo company"）が交絡しうるので、
デモを抜けて aaa / bbb だけで 測り直したのが下2行。結論は同じ。

**新UIのURLはSPAの表示を変えるだけで、旧`.aspx`が使うセッションの組織は動かない。**
この案は捨てること。再挑戦する必要はない。

#### 予防で残っている道（未着手）

Xero自身の**組織切替メニュー**なら確実にセッションが動く（それが正規の手順なので）。
DevToolsのNetworkタブを開いた状態で組織を切り替え、飛んでいるリクエストを採取すれば
再現できるかもしれない。⚠️ **URLを推測しないこと** — 28件中15件が死んでいた原因がそれ。

ただし採算は微妙:

- 組織切替はXero側の状態を変える操作。パレットの遷移のたびに勝手に叩くのは副作用が読めない
- Xeroが変えたらパレット全体が壊れる
- **検出だけでも「他社の帳簿に入力してしまう」という最悪の事故は防げている**

v0.9.0は検出で出し、予防は必要が確認できてからでよい。

#### 実機確認の結果（2026-08-12）

- ✅ **正しく着地したときは何も出ない**（誤検出なし）
- ✅ **不一致のとき赤いバーが出る。** `/AccountsPayable/Edit.aspx`（New Bill）で確認
  ```
  警告バー : 出ている ✅
  マーカー : 消えている → 判定は結論を出した
  組織リンク: ある → !9j571
  ```
- 🔧 実機で**2行目の下端が切れていた**（両組織名が入る、一番読ませたい行）。
  Xeroのスタイルがbodyの子として届くため、寸法を全部明示して修正。
  `line-height:1` を外から当てる敵対的CSSでも切れないことを確認
- 🔧 `chrome?.storage?.local?.get(...).then(...)` は、呼べないとき `undefined.then` で
  TypeErrorになり**警告ごと消えていた**。名前が引けなくても組織IDで必ず出すよう修正

**強制的に不一致を起こす方法**（セッションの再現に頼らず検出だけを試す）:

```js
sessionStorage.setItem('xp_nav_intent', JSON.stringify({ org: '!N9Q4R', at: Date.now() })); location.reload();
```

`at` は60秒で失効するので、setItemとreloadは1行にまとめること。拡張の再読み込みも忘れずに。

#### まだ確かめていないこと

- Switchボタンで実際に意図した組織へ戻れるか（ボタンの遷移先は自動テストで確認済み）
- 別組織に着地したあと `xp_bank_accounts` が汚れないこと（実装済み・実機未確認）
- 実際にどの程度の頻度で起きるか（テスト2＝タブ操作での再現は未実施）

### ✅ 解決: `brSay()` が見えない（2026-08-12）

**原因は寸法だった。** Xeroのスタイルがbodyの子として届き、要素を潰していた。
組織警告バーで同じ症状を踏み、`box-sizing` / `height:auto` / `min-height` / `line-height` /
`overflow:visible` を全部明示して解決したので、`#xp-br-notice` にも同じ手当てをした。

QA 44（銀行照合のトラッキング必須化）の実機確認で、琥珀色の帯が**見えることを確認済み**。

⚠️ 教訓: **Xeroのページに自前の要素を出すときは、寸法を全部明示すること。**
`position:fixed` にするだけでは足りない。2回同じところで詰まった。

**残: 元の症状（候補を2件選んで合計が合わないときのReconcile無効）での再確認。**
QA 7の1-10撮影と同時にやればよい。
