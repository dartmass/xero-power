...
| 40 | 通知を**押した**ときの遷移 | ⏳ | **QA29の穴**。29は通知が出るところまでで押していなかった。テストA（テスト通知を押す）・B（実データの増加通知を押すとBills一覧が新規タブで開く）を実機で確認。残るはC＝権限を後から許可しSWを再起動しない並び |
...
- ⬜ **C**: 権限を後から許可し、Service Workerを再起動しないまま通知を押す並び。**今回の修正が効く場面はここ**なので未確認
